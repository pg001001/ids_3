...
scan_information() {
    ...
    # endpoints
    grep -r --color=always -i -E "admin|auth|api|jenkins|corp|dev|stag|stg|prod|sandbox|swagger|aws|azure|uat|test|vpn|cms" "${base_dir}/js.txt" >> "${base_dir}/important_http_urls.txt"
    
    # aws s3 files and aws access key
    #grep -r --color=always -i -E "aws|s3" "${base_dir}/js.txt" >> "${base_dir}/information/aws_s3_files.txt"
    #grep -r --color=always -i -E "AKIA[0-9A-Z]{16}" "${base_dir}/allurls.txt" >> "${base_dir}/information/aws_access_keys.txt"
    ...
    # mfa urls - General OAuth pattern
    grep -r --color=always -i -E "oauth_consumer_key|oauth2|oauth[_-]token|oauth[_-]access[_-]token|bearer[_-]token" "${base_dir}/allurls.txt" >> "${base_dir}/information/mfa_links.txt"

    # tokens urls
    #grep -r --color=always -i -E "request_token|token|profileToken|access_token" "${base_dir}/allurls.txt" >> "${base_dir}/information/tokens.txt"
    ...
    #grep -r --color=always -i -E "verify-emails|verify-account|reset[_-]password|verify[_-]email|confirmation[_-]token" "${base_dir}/allurls.txt" >> "${base_dir}/information/verify-emails.txt"
    ...
    # credentials
    grep -r --color=always -i -E "register|signin|signup|forgotpassword|forgot-password|login|profile|alter-credentials|access" "${base_dir}/allurls.txt" >> "${base_dir}/information/credentials.txt"

    # sensitive information personal informations url - general info, ip address, phone numbers, credit cards, ssn numbers, 
    ...
    #grep -r --color=always -i -E "([0-9]{1,3}\.){3}[0-9]{1,3}" "${base_dir}/allurls.txt" >> "${base_dir}/information/ip_addresses.txt"
    #grep -r --color=always -i -E "[\+]?([0-9]{1,3})?[-.\s]?([0-9]{1,4})?[-.\s]?([0-9]{1,4})[-.\s]?([0-9]{1,9})" "${base_dir}/allurls.txt" >> "${base_dir}/information/phone_numbers.txt"
    #grep -r --color=always -i -E "([0-9]{4}[-.\s]?){3}[0-9]{4}" "${base_dir}/allurls.txt" >> "${base_dir}/information/credit_card_numbers.txt"
    #grep -r --color=always -i -E "[0-9]{3}[-\s]?[0-9]{2}[-\s]?[0-9]{4}" "${base_dir}/allurls.txt" >> "${base_dir}/information/ssn_numbers.txt"
    ...
    grep -r --color=always -i -E "\.xls|\.xml|\.xlsx|\.json|\.pdf|\.sql|\.doc|\.docx|\.pptx|\.zip|\.tar.gz|\.tgz|\.bak|\.7z|\.rar|attachment" "${base_dir}/allurls.txt" >>  "${base_dir}/information/sensitive_files.txt"
    
    # search sensitive paths
    grep -r --color=always -i -E "\.env|\.config|\.ini|password|private[_-]key" "${base_dir}/allurls.txt" >> "${base_dir}/information/sensitive_paths.txt"

    # search database connections strings (for various databases like MySQL, PostgreSQL, etc)
    grep -r --color=always -i -E "(mysql|postgres|mongodb|oracle|db2|sqlserver):\/\/[a-zA-Z0-9]+" "${base_dir}/allurls.txt" >> "${base_dir}/information/db_connection_strings.txt"
